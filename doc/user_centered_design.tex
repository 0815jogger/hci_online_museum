...
\subsubsection{Prototyp Vorschlag IIII} \label{chapt:paperprotoIIII}

In dieser Variante der Papierprototypen wurde als zentrales Element ein tangible Interface verwendet. Zusätzlich zu dem TI, gibt es eine AR Brille, ein Exponat und einen QR-Code. Die Brille ist in der Lage den QR-Code zu scannen um das Exponat virtuell auf der AR-Brille anzuzeigen. Das TI ist ein Kugelförmiges Gerät mit diversen verbauten Sensoren. Diese sind ein Gyrosensor, ein Beschleunigungssensor und ein Drucksensor. Der Gyrosensor dient zur Bestimmung der Rotation des TI wohingegen der Beschleunigungssensor eine Schüttel-Geste ermöglicht. Der Drucksensor ermöglicht das drücken des Interfaces und erlaubt ein weiteres Element im Mapping der Steuerung.
...
Dieser Usecase vernachlässigt hauptsächlich den informativen Teil wohin gegen die Userexperience im Vordergrund stehen soll.

\begin{itemize}
	\item Sobald die AR-Brille aufgesetzt und eingeschaltet wurde, ist diese in der Lage einen QR-Code zu scannen.
	\subitem Der Nutzer muss sich lediglich vor dem QR-Code eines Exponates stellen und diesen anschauen. 
	\item Nun wird durch einen hörbaren positiv klingenden Ton das erfolgreiche scannen des Codes vermittelt.
	\item Das virtuelle Objekt wird nun vor den Augen des Nutzers angezeigt.
	\subitem Das virtuelle Objekt wird, unabhängig von der Kopfposition, immer vor den Augen des Nutzers dargestellt.
	\subitem Dadurch kann der Nutzer das Objekt nicht verlieren
	\item Der Nutzer hält nun das Tangible Interface in seiner Hand
	\subitem durch drehen, drücken und schütteln ist nun möglich mit dem virtuellen Objekt zu interagieren
	\item schüttelt der Nutzer das TI wird das Objekt nun wieder entfernt.
	\item durch ausschalten oder absetzen der Brille kann der Nutzer das Programm beenden.
\end{itemize}
